...
function DisplayGameSettingsPage ( $CurrentUser )
{
	...
	if ($_POST['opt_save'] == "1")
	{
		...
		$game_config['server_notice']         = addslashes( $_POST['server_notice'] );
		$Log	.=	$lang['log_sett_notics'].": ".$_POST['server_notice']."\n";
		...
		doquery("UPDATE {{table}} SET `config_value` = '". $game_config['server_notice']           ."' WHERE `config_name` = 'server_notice';", 'config');
