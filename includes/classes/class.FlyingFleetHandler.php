			...
			if ($FleetRow['fleet_end_stay'] < time())
			{
				...
				foreach ($farray as $Item => $Group)
				{
					if ($Group != '')
					{
						...
						if(!isset($FleetCapacity))$FleetCapacity=0;
						if(!isset($FleetPoints))$FleetPoints=0;
				...
				if ($Hasard < 2)
				{
					...
					else if ($LostAmount ==0)
					{
						doquery("UPDATE {{table}} SET `fleet_mess` = '1' WHERE `fleet_id` = ". $FleetRow["fleet_id"], 'fleets');
					SendSimpleMessage ( $FleetOwner, '', $FleetRow['fleet_end_stay'], 15, $MessSender, $MessTitle, $lang['sys_expe_blackholl_nothing'] );
					}
					else
					{
						foreach ($LaFlotte as $Ship => $Count)
						{
							if(!isset($NewFleetArray))$NewFleetArray="";
